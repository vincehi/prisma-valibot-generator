...
function buildSchemasForModel(model, enums) {
    ...
    lines.push(...buildObjectSchema(`${model.name}Schema`, model.fields, enums, (f) => {
        if (!f.isRequired) {
    ...
    const createFields = model.fields.filter((f) => isScalarOrEnum(f) &&
        ...
        !f.isId &&
        !f.isUpdatedAt);
    lines.push(...buildObjectSchema(`Create${model.name}Schema`, createFields, enums, (f) => {
        if (f.isRequired) {
            return mapFieldToValibot(f, enums);
        }
    ...
    lines.push(...buildObjectSchema(`Update${model.name}Schema`, updateFields, enums, (f) => `v.optional(${mapFieldToValibot(f, enums)})`));
    return lines.join("\n");
}
function buildEnumsFile(enums) {
    ...
    for (const enm of enums) {
        ...
        lines.push(`export type ${enumName}Type = v.InferOutput<typeof ${schemaName}>;`);
        lines.push("");
    }
    return lines.join("\n");
}
